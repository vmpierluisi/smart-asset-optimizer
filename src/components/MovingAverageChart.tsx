...
export interface MovingAverageChartProps {
  data: SMAData | null;
  data50?: SMAData | null; // Optional 50-period MA data
  data200?: SMAData | null; // Optional 200-period MA data
  onTimeframeChange?: (timeframe: string) => void;
  timeframe?: string;
  /** Label shown in the UI ("SMA" or "EMA"). The math is identical. */
  maLabel?: string;
}
...
export const MovingAverageChart: React.FC<MovingAverageChartProps> = ({
  ...
  useEffect(() => {
    ...
    if (data50) console.log('SMA-50 Data:', data50);
    if (data200) console.log('SMA-200 Data:', data200);

    // Create maps of dates to MA values
    const ma20Map = new Map();
    const ma50Map = new Map();
    const ma200Map = new Map();
    ...
    if (data.values && Array.isArray(data.values) && data.values.length > 0) {
      data.values.forEach(item => {
        ...
        ma20Map.set(dateStr, parseFloat(item.ma));
      });
    } else {
      console.error('No SMA-20 values in data:', data);
      setError('No SMA-20 data available');
      return;
    }
    
    // Process SMA-50 data if available
    if (data50?.values && Array.isArray(data50.values) && data50.values.length > 0) {
      data50.values.forEach(item => {
        const dateStr = item.datetime.split(' ')[0];
        ma50Map.set(dateStr, parseFloat(item.ma));
      });
    }
    
    // Process SMA-200 data if available
    if (data200?.values && Array.isArray(data200.values) && data200.values.length > 0) {
      data200.values.forEach(item => {
        const dateStr = item.datetime.split(' ')[0];
        ma200Map.set(dateStr, parseFloat(item.ma));
      });
    }
    ...
    const combined = priceData.data.map(pricePoint => {
      ...
      const ma20Value = ma20Map.get(date);
      const ma50Value = ma50Map.get(date);
      const ma200Value = ma200Map.get(date);
      ...
      return {
        ...
        ma20: ma20Value, // SMA-20 value
        ma50: ma50Value, // SMA-50 value
        ma200: ma200Value, // SMA-200 value
    ...
    if (combined.length === 0) {
      setError('No matching data between price and SMA');
      return;
    }
        ...
        <ResponsiveContainer width="100%" height="100%">
          ...
          >
            ...
            <YAxis 
              ...
              domain={[
                (dataMin: number) => {
                  // Find the minimum of low values and MA values
                  const minValues = [
                    ...combinedData.map(item => item.low),
                    ...combinedData.filter(item => item.ma20 !== undefined).map(item => item.ma20)
                  ];
                  
                  // Include SMA-50 values if available
                  if (data50) {
                    minValues.push(...combinedData.filter(item => item.ma50 !== undefined).map(item => item.ma50));
                  }
                  
                  // Include SMA-200 values if available
                  if (data200) {
                    minValues.push(...combinedData.filter(item => item.ma200 !== undefined).map(item => item.ma200));
                  }
                  ...
                  // Include SMA-50 values if available
                  if (data50) {
                    maxValues.push(...combinedData.filter(item => item.ma50 !== undefined).map(item => item.ma50));
                  }
                  
                  // Include SMA-200 values if available
                  if (data200) {
                    maxValues.push(...combinedData.filter(item => item.ma200 !== undefined).map(item => item.ma200));
                  }
                ...
                (dataMax: number) => {
                  // Find the maximum of high values and MA values
                  const maxValues = [
                    ...combinedData.map(item => item.high),
                    ...combinedData.filter(item => item.ma20 !== undefined).map(item => item.ma20)
                  ];
                  ...
                  return (
                    <div className="custom-tooltip bg-background border border-border p-3 rounded shadow-md">
                      ...
                      <div className="grid grid-cols-2 gap-x-4 gap-y-1 mt-1 text-sm">
                        ...
                        <div>{maLabel}-20:</div>
                        <div className="text-right">{data.ma20 ? formatOHLC(data.ma20) : 'N/A'}</div>
                        {data.ma50 !== undefined && (
                          <>
                            <div>{maLabel}-50:</div>
                            <div className="text-right">{formatOHLC(data.ma50)}</div>
                          </>
                        )}
                        {data.ma200 !== undefined && (
                          <>
                            <div>{maLabel}-200:</div>
                            <div className="text-right">{formatOHLC(data.ma200)}</div>
                          </>
                        )}
                      </div>
                    </div>
            ...
            {data50 && (
              <Line
                type="monotone"
                dataKey="ma50"
                stroke="#3b82f6" // Blue color for 50-day SMA
                name={`50-Day ${maLabel}`}
                dot={false}
                strokeWidth={2}
                connectNulls={true}
            ...
            {data200 && (
              <Line
                type="monotone"
                dataKey="ma200"
                stroke="#8b5cf6" // Purple color for 200-day SMA
                name={`200-Day ${maLabel}`}
                dot={false}
                strokeWidth={2}
                connectNulls={true}
                yAxisId={0}
