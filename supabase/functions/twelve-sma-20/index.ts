...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching 20-day SMA for ${symbol}`);
    ...
    const apiUrl = `https://api.twelvedata.com/ma?symbol=${encodeURIComponent(symbol)}&interval=1day&ma_type=SMA&time_period=20&dp=2&start_date=${startDateStr}&end_date=${endDateStr}&apikey=${TWELVE_DATA_API_KEY}`;
    ...
    console.error('Error in twelve-sma-20 function:', error);
