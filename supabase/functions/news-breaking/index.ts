...
console.log('News breaking function started')
...
async function verifyApiKey(apiKey: string): Promise<boolean> {
  try {
    ...
    const testUrl = new URL(MARKETAUX_BASE_URL);
    testUrl.searchParams.append('api_token', apiKey);
...
serve(async (req: Request) => {
  ...
  try {
    ...
    if (!MARKETAUX_DATA_API_KEY) {
      console.error('MARKETAUX_DATA_API_KEY environment variable not found');
      throw new Error('Missing Marketaux API key')
    }
    
    console.log('Using API key (length):', MARKETAUX_DATA_API_KEY.length);
    // Just log the first and last 4 chars for debugging, without exposing full key
    console.log('API key preview:', `${MARKETAUX_DATA_API_KEY.substring(0, 4)}...${MARKETAUX_DATA_API_KEY.substring(MARKETAUX_DATA_API_KEY.length - 4)}`);
    
    // Verify API key on each request (could be cached for production)
    const isValidKey = await verifyApiKey(MARKETAUX_DATA_API_KEY);
    if (!isValidKey) {
      return new Response(
        JSON.stringify({ 
          error: 'Marketaux API key is invalid or expired. Please update your API key.',
    ...
    try {
      const { symbol: reqSymbol, sortBy: reqSortBy } = await req.json();
      ...
      const cacheKey = `breaking-${symbol}-${sortBy}`;
      
      // Check if we have a valid cache entry
      const cacheEntry = cache.get(cacheKey);
      if (cacheEntry && (Date.now() - cacheEntry.timestamp) < CACHE_DURATION) {
        console.log(`Using cached breaking news for ${cacheKey}`);
    ...
    url.searchParams.append('limit', '3'); // As per requirements
    url.searchParams.append('language', 'en'); // As per requirements
    ...
    if (!response.ok) {
      ...
      console.error(`Failed to fetch breaking news: ${response.status} ${errorText}`);
      
      if (response.status === 429) {
        // Rate limit exceeded - return a more helpful error
        console.log('Rate limit exceeded, returning cached breaking news if available');
        
        // Check if we have ANY cached data to return as fallback
        const fallbackEntries = Array.from(cache.entries())
          .filter(([key, entry]) => key.startsWith('breaking-') && entry.data && entry.data.data && entry.data.data.length > 0)
        ...
        if (fallbackEntries.length > 0) {
          ...
          console.log(`Using fallback breaking news cache: ${fallbackKey}`);
          
          // Add a warning to the response
          const responseWithWarning = {
            ...fallbackEntry.data,
            warning: 'API rate limit exceeded. Showing cached breaking news.',
            cached: true,
    ...
    if (fallbackEntries.length > 0) {
      // Return the most recent cached data with a warning
      const [fallbackKey, fallbackEntry] = fallbackEntries[0];
      console.log(`Server error, using fallback breaking news cache: ${fallbackKey}`);
      
      // Add a warning to the response
      const responseWithWarning = {
        ...fallbackEntry.data,
        warning: 'Server error occurred. Showing cached breaking news.',
        cached: true,
        cacheTime: new Date(fallbackEntry.timestamp).toISOString()
      };
