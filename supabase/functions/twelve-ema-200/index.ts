...
// Interface for Twelve Data API EMA response
interface TwelveDataEMAResponse {
  meta: {
    symbol: string;
    interval: string;
    currency: string;
...
serve(async (req) => {
  ...
  try {
    ...
    console.log(`Fetching 200-day EMA for ${symbol}`);
    ...
    const apiUrl = `https://api.twelvedata.com/ma?symbol=${encodeURIComponent(symbol)}&interval=1day&ma_type=EMA&time_period=200&dp=2&start_date=${startDateStr}&end_date=${endDateStr}&apikey=${TWELVE_DATA_API_KEY}`;
    ...
    const emaData = responseData as TwelveDataEMAResponse;
    
    // Ensure the response has all expected properties
    const response = {
      symbol: emaData.meta.symbol,
      meta: emaData.meta,
      values: emaData.values || [],
      status: emaData.status
    };
    ...
    console.error('Error in twelve-ema-200 function:', error);
